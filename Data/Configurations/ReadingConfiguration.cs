...
namespace TheSeer.Data.Configurations
{
    internal class ReadingConfiguration : IEntityTypeConfiguration<Reading>
    {
        public void Configure(EntityTypeBuilder<Reading> builder)
        {
            ...
            builder.HasOne(r => r.Spread).WithMany(s => s.Readings).HasForeignKey(r => r.SpreadId).IsRequired();

            builder.HasMany(r => r.JournalEntries).WithOne(je => je.Reading).HasForeignKey(je => je.ReadingId);
            builder.HasMany(r => r.DrawnCards).WithOne(dc => dc.Reading).HasForeignKey(dc => dc.ReadingId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}
